...
- 📄 **整页提取** - 一键提取整个页面内容
- 🧹 **正文容器识别** - 整页提取时自动收敛到正文容器，避开站点 UI 残渣（微信公众号、掘金、CSDN、知乎专栏、简书）
- 🚫 **噪声过滤** - 自动跳过占位图片（data URI SVG / 1×1 透明图）、空图片、`javascript:` 伪链接、空链接
...
- 标题 (h1-h6)
- 段落（内联保留链接、行内代码、加粗、斜体、图片）
- 代码块（保留语言标识）
- 有序/无序列表（支持嵌套缩进）
- 表格（识别 `<thead>` 表头，无表头时不误造分隔行）
- 图片（保留 src 和 alt，过滤占位图）
- 链接（保留文本和 href，过滤 `javascript:` 伪链接）
...
**智能元素提取模式：**
1. 按下快捷键（默认 `Alt+Shift+X` / Mac: `Command+Shift+X`）
...
- 首次使用需要刷新页面才能生效
- 某些页面可能因安全策略限制而无法使用
- 图标文件已包含在 `icons/` 目录中（16x16, 48x48, 128x128 PNG）
...
├── lib/jszip.min.js # ZIP 依赖
└── icons/           # 图标文件夹
```

提取算法为纯函数，`test/extract-test.html` 提供断言自测，浏览器打开即可运行。
